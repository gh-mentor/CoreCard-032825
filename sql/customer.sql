/*
This T-SQL script defines the schema and a stored procedures for a small financial institution's database. 
Details:
...
- Each table must record it create date and update date.
...
Steps:
1) Check if the database 'CreditCareDB' exists, if it does exist, drop it and create a new one.

2) Set the default database to 'CreditCareDB'.

3) Create  the Customer table which  stores customer information:
...
4) Create the Account table which  stores account information:
...
5) Create the  Transfer table which stores transaction information:
...
6) Create the  CreditCard table which stores credit card information:
- CreditCardID: Primary key, auto-incremented.
- CustomerID: Foreign key referencing CustomerID in the Customer table, cannot be null.
- CardNumber: Unique credit card number, cannot be null.
- CardType: Type of credit card (Visa or MasterCard), cannot be null.
- ExpiryDate: Expiry date of the credit card, cannot be null.
- Foreign Key Constraint: Ensures referential integrity with the Customer table.

7) Create the InsertCustomerAndAccount stored procedure which  inserts a new customer, account, and initial transaction:
